...
static void *buddy_alloc_impl(IAllocator *self, size_t size) {
  ...
  int lvl = want;
  while (lvl <= c->max_level && c->free_lists[lvl] == NULL)
    lvl++;
  if (lvl > c->max_level)
    return NULL;
...
static void buddy_free_impl(IAllocator *self, void *ptr) {
  ...
  if (p < c->buffer || p >= c->buffer + c->total_size)
    return;
...
IAllocator create_buddy_alloc(BuddyCtx *ctx, void *buffer, size_t total_size,
                              size_t min_block) {
  if (!is_pow2(total_size))
    total_size = pow2_floor(total_size);
  if (min_block < sizeof(BuddyNode))
    min_block = sizeof(BuddyNode);
  if (!is_pow2(min_block))
    min_block = pow2_floor(min_block);
